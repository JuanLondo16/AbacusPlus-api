...
def fake_chunk_repo():
    ...
    repo.search_similar = MagicMock(return_value=[
        {"id": 1, "source_type": "invoice", "source_id": 10, "content": "Factura A", "similarity": 0.92},
        {"id": 2, "source_type": "invoice", "source_id": 11, "content": "Factura B", "similarity": 0.85},
    ])
    return repo
...
class TestSearchChunksUseCase:
    async def test_returns_results(self, fake_chunk_repo, fake_embedding_service):
        use_case = SearchChunksUseCase(chunk_repo=fake_chunk_repo, embedding_service=fake_embedding_service)
