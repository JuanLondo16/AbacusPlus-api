...
def test_validate_registered_accounts_accepts_active_registered_codes():
    ...
    validated = use_case._validate_registered_accounts(entries)
...
def test_validate_registered_accounts_rejects_unregistered_codes():
    use_case = _use_case([{"code": "220500", "name": "Proveedores nacionales"}])
    entries = [
    ...
    with pytest.raises(ValueError, match="999999"):
        use_case._validate_registered_accounts(entries)
