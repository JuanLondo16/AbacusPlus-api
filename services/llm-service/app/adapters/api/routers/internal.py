...
def _run_structural_migrations(engine) -> None:
    ...
    with engine.connect() as conn:
        ...
        conn.execute(
            text("ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS name VARCHAR(100)")
        )
        conn.execute(text("ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS date DATE"))
        conn.execute(
            text("ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS ref VARCHAR(200)")
        )
        conn.execute(
            text("ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS move_type VARCHAR(20)")
        )
        conn.execute(
            text("ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS state VARCHAR(20)")
        )
        conn.execute(
            text("ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS journal_id INTEGER")
        )
        conn.execute(
            text(
                "ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS journal_name VARCHAR(100)"
            )
        )
        conn.execute(
            text("ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS partner_id INTEGER")
        )
        conn.execute(
            text(
                "ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS partner_name VARCHAR(200)"
            )
        )
        conn.execute(
            text("ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS partner_vat VARCHAR(50)")
        )
        conn.execute(
            text(
                "ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS currency_name VARCHAR(10)"
            )
        )
        conn.execute(
            text(
                "ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS amount_untaxed NUMERIC(18,2) NOT NULL DEFAULT 0"
            )
        )
        conn.execute(
            text(
                "ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS amount_tax NUMERIC(18,2) NOT NULL DEFAULT 0"
            )
        )
        conn.execute(
            text(
                "ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS amount_total NUMERIC(18,2) NOT NULL DEFAULT 0"
            )
        )
        conn.execute(text("ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS narration TEXT"))
        conn.execute(
            text("ALTER TABLE accounting_entries ADD COLUMN IF NOT EXISTS batch_id VARCHAR(36)")
        )
        # LLM-origin columns
        conn.execute(
        ...
        conn.execute(
            text(
                "ALTER TABLE accounting_entry_lines ADD COLUMN IF NOT EXISTS source_move_id INTEGER"
            )
        )
        conn.execute(
            text(
                "ALTER TABLE accounting_entry_lines ADD COLUMN IF NOT EXISTS sequence INTEGER NOT NULL DEFAULT 0"
            )
        )
        conn.execute(
            text(
                "ALTER TABLE accounting_entry_lines ADD COLUMN IF NOT EXISTS amount_currency NUMERIC(18,2) NOT NULL DEFAULT 0"
            )
        )
        conn.execute(
            text("ALTER TABLE accounting_entry_lines ADD COLUMN IF NOT EXISTS date_maturity DATE")
        )
        conn.execute(
            text(
                "ALTER TABLE accounting_entry_lines ADD COLUMN IF NOT EXISTS extracted_at TIMESTAMP NOT NULL DEFAULT NOW()"
            )
        )
        conn.execute(
