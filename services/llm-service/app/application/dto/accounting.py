...
class GenerateAccountingRequest(BaseModel):
    document_id: int = Field(
        ..., description="ID del documento (factura) para el cual generar el asiento.", examples=[1]
    )
    top_k: int = Field(
        default=5,
        ge=1,
        le=10,
        description="Número de chunks similares a recuperar del RAG como contexto (incluye asientos históricos y facturas de referencia).",
    )
    model: str = Field(
        default="gpt-4o-mini",
    ...
    model_config = {
        "json_schema_extra": {"example": {"document_id": 1, "top_k": 5, "model": "gpt-4o-mini"}}
    }


class EntryLine(BaseModel):
    cuenta: str = Field(
        ..., description="Código de cuenta PUC (Plan Único de Cuentas).", examples=["220500"]
    )
    nombre: str = Field(
        ..., description="Nombre de la cuenta PUC.", examples=["Proveedores nacionales"]
    )
    debito: float = Field(
        ..., description="Valor al débito. Cero si la cuenta va al crédito.", examples=[0.0]
    )
    credito: float = Field(
        ..., description="Valor al crédito. Cero si la cuenta va al débito.", examples=[47900.0]
    )
    tercero: Optional[str] = Field(
        default=None,
        description="NIT del tercero relacionado con el movimiento.",
        examples=["1026288579"],
    )
    centro_costo: Optional[str] = Field(
        default=None, description="Código del centro de costo. Vacío si no aplica."
    )
    descripcion: Optional[str] = Field(
        default=None,
        description="Descripción del movimiento contable.",
        examples=["Causación factura FE7674"],
    )
...
class AccountingEntryResponse(BaseModel):
    ...
    system_prompt_id: Optional[int] = Field(
        None, description="ID del system prompt utilizado para generar el asiento."
    )
    lines: list[EntryLineResponse] = Field(
        default_factory=list,
        description="Líneas del asiento contable (partida doble), cada una como registro independiente.",
    )
    model_used: Optional[str] = Field(
        None, description="Modelo de OpenAI que generó el asiento.", examples=["gpt-4o-mini"]
    )
    status: str = Field(
        ...,
        description="Estado del asiento: `generated` si fue exitoso, `error` si falló.",
        examples=["generated"],
    )
    error_message: Optional[str] = Field(
        None, description="Mensaje de error en caso de fallo. Null si el estado es `generated`."
    )
    rag_context: Optional[list[dict]] = Field(
        None,
        description="Chunks RAG usados para inferir la distribución contable (cuentas PUC). No afectan los valores monetarios del asiento.",
    )
...
class DocumentWithAccountingResponse(BaseModel):
    document: dict = Field(
        ..., description="Datos completos del documento (factura electrónica DIAN)."
    )
...
class RecalculateAccountingDocumentRequest(BaseModel):
    ...
    top_k: int = Field(
        default=5, ge=1, le=10, description="Número de chunks RAG a recuperar como contexto."
    )
    model: str = Field(
        default="gpt-4o-mini",
        description="Modelo de OpenAI a utilizar.",
...
class RecalculateAccountingItemResult(BaseModel):
    ...
    status: str = Field(
        ...,
        description="Resultado: `generated` si el asiento fue creado o `error` si falló.",
        examples=["generated", "error"],
    )
    accounting_entry_id: Optional[int] = Field(
        None, description="ID del asiento contable recién creado. Null si hubo error."
    )
    error_message: Optional[str] = Field(
        None, description="Mensaje de error cuando `status` es `error`."
    )
...
class RecalculateDocumentBody(BaseModel):
    ...
    model_config = {"json_schema_extra": {"example": {"top_k": 5, "model": "gpt-4o-mini"}}}
