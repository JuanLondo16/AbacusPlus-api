...
def get_sync_chart_accounts_use_case(db: Session = Depends(get_tenant_db)) -> SyncChartAccountsUseCase:
...
def get_send_journal_entry_use_case(db: Session = Depends(get_tenant_db)) -> SendJournalEntryUseCase:
    credentials = ManageCredentialsUseCase(IntegrationCredentialRepository(db))
