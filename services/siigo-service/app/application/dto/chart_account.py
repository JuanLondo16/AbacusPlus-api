...
class SyncChartAccountsRequest(BaseModel):
    account_key: str = Field(
        "default",
        description="Cuenta/empresa conectada a sincronizar.",
        examples=["empresa-principal"],
    )
    page_size: int = Field(
        100, ge=1, le=500, description="Cantidad de registros por pagina.", examples=[100]
    )
...
class ChartAccountResponse(BaseModel):
    ...
    external_id: Optional[str] = Field(
        None, description="ID externo en SIIGO cuando viene en la respuesta.", examples=["12345"]
    )
    code: str = Field(..., description="Codigo contable.", examples=["510505"])
    name: str = Field(
        ..., description="Nombre de la cuenta contable.", examples=["Gastos de personal"]
    )
    account_type: Optional[str] = Field(
        None, description="Tipo/clase de cuenta si SIIGO lo retorna.", examples=["Expense"]
    )
    level: Optional[int] = Field(None, description="Nivel jerarquico de la cuenta.", examples=[4])
    parent_code: Optional[str] = Field(
        None, description="Codigo de la cuenta padre.", examples=["5105"]
    )
    accepts_movements: Optional[bool] = Field(
        None, description="Indica si permite movimientos contables.", examples=[True]
    )
    ...
    raw_payload: dict[str, Any] = Field(
        ..., description="Payload completo retornado por SIIGO para trazabilidad."
    )
...
class SyncChartAccountsResponse(BaseModel):
    synced: int = Field(
        ..., description="Cantidad de cuentas creadas o actualizadas.", examples=[42]
    )
    accounts: list[ChartAccountResponse] = Field(
        ..., description="Cuentas actualmente almacenadas para la integracion."
    )
