...
from app.infrastructure.persistence.repositories.purchase_invoice_parameter_repository import PurchaseInvoiceParameterRepository
...
class ManagePurchaseInvoiceParametersUseCase:
    ...
    def create(self, request: PurchaseInvoiceParameterCreate):
        if request.provider.lower() != "siigo":
            raise ValidationException("Only SIIGO purchase invoice parameters are supported for now")
