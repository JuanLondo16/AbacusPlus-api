...
class RegisterTenantRequest(BaseModel):
    ...
    display_name: str = Field(..., description="Nombre visible de la empresa.", examples=["IKBO SAS"])
    email_domain: Optional[str] = Field(
        None,
        description="Dominio de email de la empresa para auto-detectar el tenant en login.",
        examples=["ikbo.co"],
    )
    admin_email: EmailStr = Field(..., description="Email del primer usuario administrador.", examples=["admin@ikbo.co"])
    admin_password: str = Field(..., min_length=8, description="Contrasena del administrador (minimo 8 caracteres).", examples=["Secure123!"])
