...
def _parse_party(party_node, additional_account_id: Optional[str]) -> dict:
    ...
    return {
        ...
        "direccion": {
            ...
            "departamento": _text(
                party_node, ".//cac:PhysicalLocation/cac:Address/cbc:CountrySubentity"
            ),
            "linea": _text(
...
def _parse_tax_subtotals(tax_total_node) -> list:
    ...
    for sub in tax_total_node.findall("cac:TaxSubtotal", _NS):
        ...
        taxes.append(
            {
                "codigo": scheme_id,
                "nombre": _text(sub, "cac:TaxCategory/cac:TaxScheme/cbc:Name")
                or _TAX_SCHEME.get(scheme_id),
                "porcentaje": percent_el.text.strip()
                if percent_el is not None and percent_el.text
                else None,
                "valor_por_unidad": per_unit_el.text.strip()
                if per_unit_el is not None and per_unit_el.text
                else None,
...
def parse_xml(xml_content: str) -> dict:
    ...
    try:
            ...
            data["items"].append(
                {
                    ...
                    "precio_unitario": price_el.text.strip()
        ...
        data["totales"] = {
            ...
            "redondeo": rounding_el.text.strip()
            if rounding_el is not None and rounding_el.text
            else None,
