...
class CostCenterResponse(BaseModel):
    ...
    external_id: Optional[str] = Field(
        None, description="ID externo en el proveedor.", examples=["13222"]
    )
    ...
    raw_payload: dict[str, Any] = Field(
        ..., description="Datos originales usados para crear/actualizar el centro."
    )
...
class ImportCostCentersResponse(BaseModel):
    imported: int = Field(
        ..., description="Cantidad de filas creadas o actualizadas.", examples=[8]
    )
    cost_centers: list[CostCenterResponse] = Field(
        ..., description="Centros almacenados despues de la importacion."
    )
