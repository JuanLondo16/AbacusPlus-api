...
class PurchaseInvoiceParameterCreate(BaseModel):
    ...
    document_id: int = Field(..., description="ID del tipo de comprobante en el proveedor.", examples=[58246])
    ...
    default_payment_id: Optional[int] = Field(None, description="ID del medio de pago.", examples=[51279])
    default_payment_due_date: Optional[date] = Field(None, description="Fecha de vencimiento por defecto si el pago la requiere.")
    default_item_type: str = Field("Account", description="Tipo de item del proveedor.", examples=["Account"])
    ...
    retentions: List[Dict[str, Any]] = Field(default_factory=list, description="Retenciones por defecto.")
    taxes: List[Dict[str, Any]] = Field(default_factory=list, description="Impuestos por defecto para items.")
    extra_payload: Dict[str, Any] = Field(default_factory=dict, description="Parametros adicionales para futuras extensiones.")
